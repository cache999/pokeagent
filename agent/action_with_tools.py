...
def action_step(memory_context, current_plan, latest_observation, frame, state_data, recent_actions, vlm):
        ...
        if battle_info:
            if 'player_pokemon' in battle_info:
                ...
                action_context.append(
                    f"Your Pokemon: {player_pkmn.get('species_name', player_pkmn.get('species', 'Unknown'))} (Lv.{player_pkmn.get('level', '?')}) HP: {player_pkmn.get('current_hp', '?')}/{player_pkmn.get('max_hp', '?')}")
            if 'opponent_pokemon' in battle_info:
                opp_pkmn = battle_info['opponent_pokemon']
                action_context.append(
                    f"Opponent: {opp_pkmn.get('species_name', opp_pkmn.get('species', 'Unknown'))} (Lv.{opp_pkmn.get('level', '?')}) HP: {opp_pkmn.get('current_hp', '?')}/{opp_pkmn.get('max_hp', '?')}")
    else:
    ...
    You are an experienced Pokemon Emerald player. Make quick, efficient decisions to progress the game.
    ...
    - Precise movement inside buildings or on ledges.
    - **Terrain:** AVOID tall grass unless grinding is explicitly required by the plan. NEVER pathfind into water without Surf.
    ...
    Return ONLY the button name(s) or tool(s) as a comma-separated list, nothing else.
    ...
    if len(actions) == 0:
        print("❌ No valid actions parsed - using default 'A'")  # TODO this is probably bad
    print("-" * 80 + "\n")
